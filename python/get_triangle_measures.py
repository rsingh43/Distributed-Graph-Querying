...
WHERE {{
	?vertex paypal:connects ?v2 .
	?v2 paypal:connects ?v3 .
	?v3 paypal:connects ?vertex .
{pattern}
}}
"""
	table = [["weight"] + attributes]
	query = query_pattern.format(select=" ".join("?{}".format(value) for value in ["vertex"] + attributes), pattern="\n".join(lines))
	#print query
