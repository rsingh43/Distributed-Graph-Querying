...
plot "betweenness_centrality.min.csv"   using (xfix($1)):(yfix(($3/3)/(347700/3)*100)) with linespoints linestyle 1 title "Betweenness" ,\
     ...
     "norm_sum_nbr_degree.min.csv"      using (xfix($1)):(yfix(($3/3)/(347700/3)*100)) with linespoints linestyle 8 title "Sum Nbr. Degree" ,\
     "norm_sum_nbr_in_degree.min.csv"   using (xfix($1)):(yfix(($3/3)/(347700/3)*100)) with linespoints linestyle 9 title "Sum Nbr. In-Degree" ,\
     "norm_sum_nbr_out_degree.min.csv"  using (xfix($1)):(yfix(($3/3)/(347700/3)*100)) with linespoints linestyle 10 title "Sum Nbr. Out-Degree"
